...
export interface LCSettings {
  ...
  baseCurrency: string;
  approvalThresholds: Array<{ minimumAmount: number; maximumAmount: number | null; approverRole: string }>;
...
export const DEFAULT_LC_SETTINGS: LCSettings = {
  ...
  baseCurrency: 'INR',
  approvalThresholds: [
    { minimumAmount: 0, maximumAmount: 1_000_000, approverRole: 'Finance Manager' },
    { minimumAmount: 1_000_000.01, maximumAmount: 5_000_000, approverRole: 'Director Finance' },
    { minimumAmount: 5_000_000.01, maximumAmount: 20_000_000, approverRole: 'Director' },
    { minimumAmount: 20_000_000.01, maximumAmount: null, approverRole: 'Managing Director' },
  ],
  ...
  mandatoryRequestDocuments: ['PURCHASE_ORDER', 'PROFORMA_INVOICE', 'LC_FORMAT_OR_TERMS'],
  mandatoryOpeningDocuments: ['APPROVED_LC_REQUEST', 'BANK_APPLICATION', 'MARGIN_CONFIRMATION'],
  mandatoryPaymentDocuments: ['HUNDI', 'PAYMENT_APPROVAL', 'BANK_INSTRUCTION'],
  mandatoryClosureDocuments: ['BANK_CLOSURE_CONFIRMATION', 'MARGIN_RELEASE_CONFIRMATION'],
};
...
export const calculateAvailableLimit = (sanctioned: number, temporary: number, utilized: number, reserved: number) =>
  round(Number(sanctioned || 0) + Number(temporary || 0) - Number(utilized || 0) - Number(reserved || 0));

export const calculateOutstanding = (accepted: number, paid: number, reversals = 0) =>
  Math.max(0, round(Number(accepted || 0) - Number(paid || 0) - Number(reversals || 0)));
...
export const calculateHundiDueDate = (baseDate: LCDate, usancePeriodDays: number) => {
...
export const lcStatusTone = (status: string) => {
  ...
  if (['CLOSED', 'PAID', 'APPROVED', 'ACCEPTED', 'FULLY_RECOVERED', 'COMPLETED'].some((token) => normalized.includes(token))) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (['REJECTED', 'CANCELLED', 'OVERDUE', 'EXPIRED', 'DISCREPANCY'].some((token) => normalized.includes(token))) return 'bg-rose-50 text-rose-700 border-rose-200';
  if (['PENDING', 'DUE', 'AWAITED', 'PARTIALLY', 'RETURNED'].some((token) => normalized.includes(token))) return 'bg-amber-50 text-amber-800 border-amber-200';
