 ...
 *   3. **The display surface** is an `<iframe sandbox>` with no `allow-scripts` and no
 *      `allow-same-origin`, carrying a Content-Security-Policy that forbids everything except
 *      inline styles and the image sources the reader allowed. Even markup that got past the first
 *      two layers could not run code, read the ERP's storage or make a request of its own.
 *
    ...
    transformTags: {
      ...
      a: (tagName, attribs) => {
        ...
        if (/^https?:\/\//i.test(href)) {
          ...
          if (rewriteLinks) next.href = interstitialHref(href, false);
          next.target = '_blank';
...
/**
 * The document the display frame renders.
 *
 * The CSP is the frame's own: no script source at all, no connections, no frames, no forms, images
 * only from `data:` (and `https:` when the reader allowed remote content). `base-uri 'none'`
 * stops a surviving `<base>` retargeting links.
 */
