...
export default function PrintProformaBillPage() {
  const params = useParams();
  const { toast } = useToast();

  const { project: projectSlug, proformaId } = params as {
    project: string;
    proformaId: string;
  };

  const [proformaBill, setProformaBill] = useState<ProformaBill | null>(null);
  ...
  useEffect(() => {
    const fetchPrintData = async () => {
      if (!projectSlug || !proformaId) return;
      ...
      try {
        ...
        const proformaDocRef = doc(db, 'projects', projectData.id, 'proformaBills', proformaId);
        const proformaDocSnap = await getDoc(proformaDocRef);
        if (!proformaDocSnap.exists()) throw new Error('Proforma Bill not found.');
        
        const billData = { id: proformaDocSnap.id, ...proformaDocSnap.data() } as ProformaBill;
        setProformaBill(billData);
    ...
    fetchPrintData();
  }, [projectSlug, proformaId, toast]);

  /* ----- Auto-print when ready ----- */
  useEffect(() => {
    if (!isLoading && proformaBill) {
      const id = setTimeout(() => {
  ...
  if (!proformaBill) {
    return <div className="p-8">Proforma Bill not found.</div>;
  }

  const workDetails = {
    orderNo: proformaBill.workOrderNo || 'N/A',
    projectName: project?.projectName || 'N/A',
    projectSite: project?.projectSite || 'N/A',
    billDate: formatDateSafe(proformaBill.date),
    billNo: proformaBill.proformaNo,
  };
  
  const subtotal = proformaBill.items.reduce((sum, item) => sum + parseFloat(String(item.totalAmount || '0')), 0);
      ...
      <div className="bg-white">
        <div id="printable-sheet">
          <div className="text-center">
            <h1 className="text-lg font-extrabold">PROFORMA / ADVANCE BILL</h1>
          </div>
          
          <div className="text-[9pt] space-y-1 mb-2 mt-4 border border-black p-2">
            <div className="grid grid-cols-2">
                <div><p><strong>Work Order No:</strong> {workDetails.orderNo}</p></div>
                <div><p><strong>Subcontractor:</strong> {proformaBill.subcontractorName}</p></div>
            </div>
          ...
          <div className="overflow-x-auto border border-black">
            <Table className="w-full table-auto text-[8pt]">
              ...
              <TableBody>
                {proformaBill.items.map((item, index) => (
                  <TableRow key={index}>
                    <TableCell className="text-center border-black">{item.boqSlNo || index + 1}</TableCell>
                ...
                <Table>
                    <TableBody>
                        ...
                        <TableRow>
                            ...
                            <TableCell className="text-right">{proformaBill.payablePercentage}%</TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell className="font-bold">Net Payable</TableCell>
                            <TableCell className="text-right font-bold">{formatCurrency(proformaBill.payableAmount)}</TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
            </div>
