...
export default function ImportBoqPage() {
  ...
  const [jsonData, setJsonData] = useState<BoqItemData[]>([]);
  ...
  const parseExcel = (fileToParse: File) => {
      ...
      reader.onload = (event) => {
          ...
          const json = XLSX.utils.sheet_to_json<BoqItemData>(worksheet);
          setJsonData(json);
          if (json.length > 0) {
  ...
  const handleImport = async () => {
    ...
    try {
        ...
        jsonData.forEach(item => {
            ...
            batch.set(docRef, {...item, projectSlug: projectSlug});
        });
        ...
        toast({
            ...
            description: `${jsonData.length} items have been imported to the BOQ.`,
        });
        setJsonData([]);
        setHeaders([]);
        setFile(null);
