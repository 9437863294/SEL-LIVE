...
export default function ViewBoqPage() {
  ...
  const [columnVisibility, setColumnVisibility] = useState<Record<string, boolean>>(
    baseTableHeaders.reduce((acc, h) => ({
      ...acc,
      [h]: ['BOQ SL No', 'ERP SL NO', 'Description', 'Unit', 'QTY', 'Category 1'].includes(h)
    }), {})
  );
  ...
  const normalizeKey = (obj: any, targetKey: string): string | undefined => {
    const foundKey = Object.keys(obj).find(k => k.toLowerCase().replace(/\s+/g, '') === targetKey.toLowerCase().replace(/\s+/g, ''));
    return foundKey;
  };
