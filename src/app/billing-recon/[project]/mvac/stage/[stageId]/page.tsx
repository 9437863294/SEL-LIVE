...
function computeTotalAmount(entry: MvacEntry | undefined): number {
  if (!entry) return 0;
...
export default function StagePage() {
  ...
  const [tasks, setTasks] = useState<MvacEntry[]>([]);
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const [stageTasksSnap, boqSnap, billsSnap] = await Promise.all([
        getDocs(query(collection(db, 'projects', pid, 'mvacEntries'), where('currentStepId', '==', stageId))),
        getDocs(query(collection(db, 'projects', pid, 'boqItems'))),
        getDocs(query(collection(db, 'projects', pid, 'bills'))),
      ]);

      setTasks(stageTasksSnap.docs.map((d) => ({ id: d.id, ...(d.data() as any) } as MvacEntry)));
  ...
  const { pendingTasks, completedTasks } = useMemo(() => {
    if (!userId || !stage) return { pendingTasks: [] as MvacEntry[], completedTasks: [] as MvacEntry[] };
  ...
  const handleDialogOpenChange = (open: boolean) => {
    if (!open) {
      ...
      setSelectedMvac(null);
    }
  };

  const handleUpdateQtyClick = (entry: MvacEntry) => {
    setSelectedMvac(entry);
    setIsUpdateQtyOpen(true);
  };

  const handleViewDetails = (entry: MvacEntry) => {
    setSelectedMvac(entry);
    setIsViewOpen(true);
  };

  const handleVerifyClick = (entry: MvacEntry) => {
    setSelectedMvac(entry);
  ...
  const handleAction = async (
    ...
    try {
      ...
      await runTransaction(db, async (tx) => {
        ...
        const preData = preSnap.data() as MvacEntry;

        let nextStep: WorkflowStep | undefined;
        let newStatus: MvacEntry['status'] = preData.status;
        ...
        if (isCompletionAction) {
          ...
          if (nextStep) {
            const serializableData = {
              ...
              mvacDate: toDateSafe((preData as any).mvacDate)?.toISOString() ?? new Date().toISOString(),
            ...
            {isLoading ? (
              ...
              data.map((entry) => {
                ...
                return (
                  ...
                  >
                    <TableCell className="whitespace-nowrap">{entry.mvacNo ?? '-'}</TableCell>
  ...
  return (
    <>
      ...
      <ViewMvacEntryDialog
        ...
        isLoading={selectedMvac ? isActionLoading === selectedMvac.id : false}
      />
