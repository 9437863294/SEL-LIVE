  ...
  const totalCertifiedQtyMap = useMemo(() => {
    ...
    jmcEntries.forEach(entry => {
        entry.items.forEach(item => {
            const s1 = (item as any)?.scope1 ?? (item as any)?.['Scope 1'] ?? '';
            const sl = (item as any)?.boqSlNo ?? (item as any)?.['BOQ SL No'] ?? '';
            if (!String(sl || '').trim()) return;
            const key = compositeKey(s1, sl);
            map[key] = (map[key] || 0) + num0((item as any)?.certifiedQty);
        });
    });
    return map;
  }, [jmcEntries]);
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        const allJmc = jmcSnaps.flatMap((snap, index) =>
          snap.docs.map((d) => ({
            ...(d.data() as Omit<JmcEntry, 'id'>),
            id: d.id,
            projectId: projectsData[index].id,
          }))
        );
        setJmcEntries(allJmc);
