...
import { Home, Folder, ShieldAlert, Settings } from 'lucide-react';
...
const SHOW_SETTINGS_CARD = false; // set true if you want a settings card in the grid too
...
export default function BillingReconPage() {
  ...
  const canViewModule = useMemo(() => {
    // If auth is loading, don't compute a false negative – defer decision
    if (isAuthLoading) return false;
    try {
      return can('View Module', 'Billing Recon');
    } catch {
      return false;
    }
  }, [can, isAuthLoading]);
  ...
  const projectItems = useMemo(
    () =>
      projects
        .filter((p) => p?.billingRequired === true)
        .map((project) => ({
          icon: Folder,
  ...
  // Top-level loading (auth or initial data fetch)
  ...
  const itemsToShow = SHOW_SETTINGS_CARD ? [...projectItems, settingsItem] : projectItems;
      ...
      {itemsToShow.length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Projects with <code>billingRequired = true</code> will appear here.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Check your project documents or update the{" "}
              <Link href="/billing-recon/settings" className="underline">
                billing settings
              </Link>
              .
            </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {itemsToShow.map((item) => (
            <BillingReconCard key={`${item.text}-${item.href}`} item={item} />
          ))}
        </div>
      )}
    </div>
