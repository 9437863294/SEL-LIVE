...
export async function POST(req: NextRequest) {
  try {
    ...
    const subject = isOver
      ? `Budget Exceeded — ${projectName} (${monthLabel})`
      : `Budget ${thresholdPct}% Alert — ${projectName} (${monthLabel})`;
...
<body style="font-family:Calibri,Arial,sans-serif;background:#f4f4f4;margin:0;padding:20px;">
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.12);">
    <div style="background:${statusColor};padding:24px 32px;">
      ...
      <h2 style="margin:6px 0 0;color:#fff;font-size:20px;">${isOver ? '🚨 Budget Exceeded' : `⚠️ Budget ${thresholdPct}% Alert`}</h2>
    </div>
    <div style="padding:24px 32px;">
      <table style="width:100%;border-collapse:collapse;margin-bottom:20px;font-size:13px;">
        ...
        ${isOver
          ? `<tr><td style="padding:8px 12px;background:#f8f8f8;border:1px solid #e0e0e0;font-weight:bold;">Over Budget By</td><td style="padding:8px 12px;border:1px solid #e0e0e0;color:#C00000;font-weight:bold;">₹${overBy.toLocaleString('en-IN')}</td></tr>`
          : `<tr><td style="padding:8px 12px;background:#f8f8f8;border:1px solid #e0e0e0;font-weight:bold;">Remaining Budget</td><td style="padding:8px 12px;border:1px solid #e0e0e0;color:#375623;font-weight:bold;">₹${remaining.toLocaleString('en-IN')}</td></tr>`
        }
      </table>
