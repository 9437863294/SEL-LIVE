...
export default function ExpenseReportsPage() {
    ...
    const [accountSummary, setAccountSummary] = useState<HeadSummary[]>([]);
    ...
    useEffect(() => {
        ...
        const deptSummaryData = allDepartments.map(dept => {
        ...
        setDepartmentSummary(deptSummaryData.sort((a, b) => b.totalAmount - a.totalAmount));

        // Calculate Project Summary
        const projSummaryData = allProjects.map(proj => {
        ...
        setProjectSummary(projSummaryData.sort((a,b) => b.totalAmount - a.totalAmount));

        // Calculate Account Summary
        const accSummaryData = allHeads.map(head => {
        ...
        setAccountSummary(accSummaryData.filter(h => h.requestCount > 0).sort((a,b) => b.totalAmount - a.totalAmount));

    }, [filters, allExpenses, allProjects, allDepartments, allHeads, allSubHeads, isLoading, canViewPage]);
            ...
            <div className="space-y-8">
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Total requested amounts and request counts per department.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader><TableRow><TableHead>Department</TableHead><TableHead className="text-right">Total Requests</TableHead><TableHead className="text-right">Total Amount</TableHead></TableRow></TableHeader>
                            <TableBody>
                                {isLoading ? Array.from({ length: 3 }).map((_, i) => <TableRow key={i}><TableCell colSpan={3}><Skeleton className="h-5 w-full" /></TableCell></TableRow>)
                                : departmentSummary.map(s => <TableRow key={s.departmentName}><TableCell className="font-medium">{s.departmentName}</TableCell><TableCell className="text-right">{s.requestCount.toLocaleString()}</TableCell><TableCell className="text-right font-semibold">{s.totalAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</TableCell></TableRow>)}
                            </TableBody>
                        </Table>
                    </CardContent>
                </Card>
                 ...
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Total requested amounts and request counts per project.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader><TableRow><TableHead>Project</TableHead><TableHead className="text-right">Total Requests</TableHead><TableHead className="text-right">Total Amount</TableHead></TableRow></TableHeader>
                            <TableBody>
                                {isLoading ? Array.from({ length: 3 }).map((_, i) => <TableRow key={i}><TableCell colSpan={3}><Skeleton className="h-5 w-full" /></TableCell></TableRow>)
                                : projectSummary.map(s => <TableRow key={s.projectName}><TableCell className="font-medium">{s.projectName}</TableCell><TableCell className="text-right">{s.requestCount.toLocaleString()}</TableCell><TableCell className="text-right font-semibold">{s.totalAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</TableCell></TableRow>)}
                            </TableBody>
                        </Table>
                    </CardContent>
                </Card>
                ...
                <div>
                    <CardHeader className="px-0">
                        <CardTitle>Head of Account Summary</CardTitle>
                        <CardDescription>A breakdown of expenses based on the chart of accounts.</CardDescription>
                    </CardHeader>
                    {isLoading ? <div className="space-y-2"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div>
                    : <Accordion type="multiple" defaultValue={accountSummary.map(s => s.headName)}>
                        {accountSummary.map(head => (
                            <AccordionItem value={head.headName} key={head.headName}>
                                <AccordionTrigger className="text-lg font-semibold hover:no-underline">
                                    <div className="flex justify-between w-full pr-4">
                                        <span>{head.headName}</span>
                                        <span className="text-right font-bold text-primary">{head.totalAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</span>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent>
                                    <Table>
                                        <TableHeader><TableRow><TableHead>Sub-Head of Account</TableHead><TableHead className="text-right">Total Requests</TableHead><TableHead className="text-right">Total Amount</TableHead></TableRow></TableHeader>
                                        <TableBody>
                                            {head.subHeads.map(sub => <TableRow key={sub.subHeadName}><TableCell>{sub.subHeadName}</TableCell><TableCell className="text-right">{sub.requestCount.toLocaleString()}</TableCell><TableCell className="text-right">{sub.totalAmount.toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}</TableCell></TableRow>)}
                                        </TableBody>
                                    </Table>
                                </AccordionContent>
                            </AccordionItem>
