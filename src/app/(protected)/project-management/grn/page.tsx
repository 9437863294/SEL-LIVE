...
/**
 * Link out to the quantity-driven goods receipt documents.
 *
    ...
    <Card className="border-border/60 bg-muted/30">
      <CardContent className="flex flex-wrap items-center justify-between gap-3 py-3">
        <p className="text-sm text-muted-foreground">
          This register shows each BOQ item&rsquo;s gate. GRN <strong>quantity</strong> is tracked on
          documents, which can cover several purchase order lines at once.
        </p>
        <div className="flex shrink-0 gap-2">
          <Button variant="outline" size="sm" asChild>
            <Link href={`/project-management/grn/documents${query}`}>View GRN documents</Link>
          </Button>
          <Button size="sm" asChild>
            <Link href={`/project-management/grn/new${query}`}>New GRN</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
