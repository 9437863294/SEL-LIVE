...
export default function JmcSummaryPage() {
  ...
  const fetchSummaryData = useCallback(async () => {
      ...
      try {
        ...
        const currentProject = context.resolveProject(allProjects) as Project | null;

        if (!currentProject) {
          console.error("Project not found for mapping:", context.mappingId);
          setIsLoading(false);
          return;
        }
  ...
  useEffect(() => {
    if (!isAuthLoading && !isResolving) {
      if(canViewPage && !notFound) {
          fetchSummaryData();
      } else {
          setIsLoading(false);
      }
    }
  }, [isAuthLoading, isResolving, notFound, canViewPage, fetchSummaryData]);
  ...
  if (isAuthLoading || isResolving || (isLoading && canViewPage && !notFound)) {
    return <JmcLoadingState blocks={2} />;
  }

  if(!canViewPage) {
    return <JmcAccessDenied description="You do not have permission to view this page." />;
  }

  if (notFound) {
    return (
      <JmcProjectNotFound description="This report could not resolve a Project Management project from the link you followed. Open it from a project's Civil execution workspace." />
    );
  }
  ...
  return (
    <JmcPageShell>
      <JmcPageHeader
        ...
        icon={BarChart3}
        backHref={context.jmcHref('reports')}
      ...
      <h2 className="text-lg font-semibold">Step-wise Report</h2>
