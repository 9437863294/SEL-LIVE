...
/**
 * Link out to the quantity-driven dispatch instruction documents.
 *
  ...
  return (
    <Card className="border-border/60 bg-muted/30">
      <CardContent className="flex flex-wrap items-center justify-between gap-3 py-3">
        <p className="text-sm text-muted-foreground">
          This register shows each BOQ item&rsquo;s gate. Dispatch Instruction <strong>quantity</strong> is tracked on
          documents, which can cover several purchase order lines at once.
        </p>
        <div className="flex shrink-0 gap-2">
          <Button variant="outline" size="sm" asChild>
            <Link href={`/project-management/dispatch-instructions/documents${query}`}>View Dispatch Instruction documents</Link>
          </Button>
          <Button size="sm" asChild>
            <Link href={`/project-management/dispatch-instructions/new${query}`}>New Dispatch Instruction</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
