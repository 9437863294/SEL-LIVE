...
export default function ProjectWiseReportPage() {
  ...
  const rows = useMemo((): ProjectRow[] => {
    ...
    filtered.forEach((r) => {
      const key = r.projectId || '__unknown__';
      if (!map[key]) {
        map[key] = {
          projectId: key,
          projectName: projectMap[key] || key,
          total: 0,
          totalAmount: 0,
          pending: 0,
          inProgress: 0,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const ws = wb.addWorksheet('Project-wise Report');
      ws.addRow(['Project', 'Total Requests', 'Total Amount', 'Pending', 'In Progress', 'Completed', 'Rejected']);
      rows.forEach((r) =>
        ws.addRow([r.projectName, r.total, r.totalAmount, r.pending, r.inProgress, r.completed, r.rejected])
      );
      ...
      a.href = url;
      a.download = 'site-fund-request-project-wise.xlsx';
      a.click();
      ...
      <div className="w-full space-y-4 p-4 sm:p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Project-wise Report</h1>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">Project-wise Report</h1>
            <p className="mt-0.5 text-sm text-slate-600">Requests and amounts grouped by project.</p>
          </div>
        </div>
        <Button
          variant="outline"
