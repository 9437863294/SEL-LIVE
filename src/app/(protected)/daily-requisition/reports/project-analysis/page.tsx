...
export default function ProjectAnalysisPage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        projectsSnap.docs.forEach((d) => {
          const data = d.data();
          nameMap[d.id] = (data.projectName as string) || (data.name as string) || d.id;
        });
  ...
  const rows = useMemo((): ProjectRow[] => {
    ...
    filtered.forEach((e) => {
      const key = e.projectId || '__unknown__';
      if (!map[key]) {
        map[key] = {
          projectId: key,
          name: projectNameMap[key] || key,
          count: 0,
          totalGross: 0,
          totalNet: 0,
          paid: 0,
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      const ws = wb.addWorksheet('Project Analysis');
      ws.columns = [
      ...
      a.href = url;
      a.download = `project-analysis-${fromDate}-to-${toDate}.xlsx`;
      a.click();
    ...
    <div className={dailyPageContainerClass}>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          },
          {
            ...
            hint: topProject ? `${topProject.count} entries` : undefined,
      ...
      <Card className="overflow-hidden border border-white/70 bg-white/70 shadow-sm backdrop-blur">
        ...
        <CardHeader className="pb-2">
          ...
          <CardDescription>
            {rows.length} project{rows.length !== 1 ? 's' : ''} · {totalEntries} entries in range
          </CardDescription>
        </CardHeader>
              ...
              <table className="w-full caption-bottom text-sm">
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr
                      key={row.projectId}
                      className="border-b border-slate-100 transition-colors hover:bg-amber-50/60"
                    >
