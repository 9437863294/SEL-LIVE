...
const getScope1 = (item: any): string =>
  String(item?.scope1 || item?.['Scope 1'] || '').trim();
const getScope2 = (item: any): string =>
  String(item?.scope2 || item?.['Scope 2'] || '').trim();
const getBoqSlNo = (item: any): string =>
  String(item?.boqSlNo ?? item?.['BOQ SL No'] ?? item?.['SL. No.'] ?? '').trim();
const compositeKey = (item: any) =>
  `${getScope1(item)}_${getScope2(item)}_${getBoqSlNo(item)}`;

export default function WorkOrderDetailsPage() {
  const params = useParams();
  const router = useRouter();
  const { project: projectSlug, workOrderId } = params as {
    project: string;
    workOrderId: string;
  };
  ...
  useEffect(() => {
    const fetchWorkOrderDetails = async () => {
      ...
      try {
        ...
        const proformaBills = proformaSnap.docs
          .map(doc => doc.data() as ProformaBill)
          .filter(pb => pb.workOrderId === workOrderId);
        ...
        bills.forEach(bill => {
          ...
          bill.items.forEach(item => {
            ...
            billedQtyMap.set(
              item.jmcItemId,
              currentQty + (parseFloat(item.billedQty as any) || 0)
            );
          });
        });

        const totalAdvanceTaken = proformaBills.reduce(
        ...
        const enriched = woData.items.map(item => {
          ...
          return {
            ...
            boqRate: boqItem && rateKey ? String((boqItem as any)[rateKey] || '0') : '0',
  ...
  const progressPercentage =
    workOrder.totalAmount > 0
      ? (financials.totalBilled / workOrder.totalAmount) * 100
      : 0;
  const netAdvanceBalance =
        ...
        <Card>
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {formatCurrency(financials.totalAdvanceTaken)}
              </span>
            </div>
            <div className="flex justify-between">
              <span>Total Advance Deducted</span>
              <span className="font-semibold text-destructive">
                -{formatCurrency(financials.totalAdvanceDeducted)}
              </span>
            </div>
            <div className="flex justify-between border-t pt-2 font-bold">
              <span>Net Advance Balance</span>
              <span>{formatCurrency(netAdvanceBalance)}</span>
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <Table className="table-fixed">
              <TableHeader>
                <TableRow>
                  <TableHead className="w-20">BOQ Sl.No</TableHead>
                  <TableHead className="w-[420px]">Description</TableHead>
                  <TableHead className="w-16">Unit</TableHead>
                  <TableHead className="w-32">BOQ Qty</TableHead>
                  <TableHead className="w-32">BOQ Rate</TableHead>
                  <TableHead className="w-28">Order Qty</TableHead>
                  <TableHead className="w-36">JMC Certified Qty</TableHead>
                  <TableHead className="w-32">Billed Qty</TableHead>
                  <TableHead className="w-32">Order Rate</TableHead>
                  <TableHead className="w-40">Total Amount</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {enrichedItems.map(item => {
                  const boqRateNum = Number(item.boqRate);
                  const boqRateDisplay =
                    !isNaN(boqRateNum) && boqRateNum !== 0
                      ? formatCurrency(boqRateNum)
                      : 'N/A';

                  return (
                    <TableRow key={item.id}>
                      <TableCell className="truncate">{item.boqSlNo}</TableCell>
                      <TableCell>
                        <span
                      ...
                      <TableCell className="truncate">{item.boqQty}</TableCell>
