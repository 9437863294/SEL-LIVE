...
export default function LoanDetailsPage() {
  ...
  const fetchLoanData = async () => {
    ...
    setIsLoading(true);
    try {
      // Loan doc (avoid duplicate `id`)
      const loanDocRef = doc(db, 'loans', loanId);
      const loanDocSnap = await getDoc(loanDocRef);

      if (loanDocSnap.exists()) {
        const rawLoan = loanDocSnap.data() as any;
        const { id: _ignoredLoanId, ...loanRest } = rawLoan ?? {};
        const loanData = { ...loanRest, id: loanDocSnap.id } as Loan;
        setLoan(loanData);
      ...
      setAccountHeads(
        headsSnap.docs.map(d => {
          const raw = d.data() as any;
          const { id: _i, ...rest } = raw ?? {};
          return { ...rest, id: d.id } as AccountHead;
        })
      );
      ...
      setSubAccountHeads(
        subHeadsSnap.docs.map(d => {
          const raw = d.data() as any;
          const { id: _i, ...rest } = raw ?? {};
          return { ...rest, id: d.id } as SubAccountHead;
        })
      );
      ...
      setDepartments(
        deptsSnap.docs.map(d => {
          const raw = d.data() as any;
          const { id: _i, ...rest } = raw ?? {};
          return { ...rest, id: d.id } as Department;
        })
      );
      ...
      setExpenseRequests(
        expenseReqsSnap.docs.map(d => {
          const raw = d.data() as any;
          const { id: _i, ...rest } = raw ?? {};
          return { ...rest, id: d.id } as ExpenseRequest;
        })
      );
    } catch (error) {
  ...
  const handleMarkAsPaidClick = (emi: EMI) => {
    setSelectedEmi(emi);
    setDialogPaidAmount(emi.emiAmount ?? 0);
    setDialogPrincipal(emi.principal ?? 0);
    setDialogInterest(emi.interest ?? 0);
  ...
  const handleEditEmiClick = (emi: EMI) => {
    ...
    setSelectedEmi(emi);
    setDialogPaidAmount(emi.paidAmount ?? 0);
    setDialogPrincipal(emi.principal ?? 0);
    setDialogInterest(emi.interest ?? 0);
    setIsPayDialogOpen(true);
  };
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      await runTransaction(db, async transaction => {
        ...
        transaction.update(loanDocRef, { totalPaid: (currentLoanData.totalPaid || 0) + paidAmountDifference });
      });
    ...
    try {
      ...
      await runTransaction(db, async transaction => {
        ...
        if (!loanDoc.exists() || !emiDoc.exists()) {
          throw new Error('Loan or EMI document does not exist!');
        }
        const currentLoanData = loanDoc.data() as Loan;
        ...
        transaction.update(loanDocRef, {
          totalPaid: (currentLoanData.totalPaid || 0) - amountToReverse,
        });
      });
  ...
  // --- helpers (single declaration to avoid "Cannot redeclare block-scoped variable 'round'") ---
  const round = (num: number) => Math.round(num);
  const formatCurrency = (amount: number) =>
    new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(round(amount));
  ...
  const formatDate = (date: any) => {
    if (typeof date === 'string') return format(new Date(date), 'dd MMM, yyyy');
    if (date && typeof date.toDate === 'function') return format(date.toDate(), 'dd MMM, yyyy');
    return 'N/A';
  };
  // ---------------------------------------------------------------------------------------------
  ...
  const handleRegenerateSchedule = () => {
    ...
    if (p > 0 && r > 0 && n > 0 && editedLoan.startDate) {
      ...
      toast({
        ...
        description: 'Loan Amount, Tenure, Rate, and Start Date are required.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleEmiScheduleChange = (index: number, field: 'principal' | 'interest', value: number) => {
    ...
    const openingBalance =
      index > 0 ? newSchedule[index - 1].closingPrincipal : (editedLoan?.loanAmount || 0);

    if (item.principal > openingBalance) {
      item.principal = round(openingBalance);
      item.emiAmount = round(item.principal + item.interest);
    }
    ...
    for (let i = index + 1; i < newSchedule.length; i++) {
      const prevClosing = newSchedule[i - 1].closingPrincipal;
      const currentItem = newSchedule[i];
      currentItem.principal = round(currentItem.emiAmount - currentItem.interest);
      if (currentItem.principal > prevClosing) currentItem.principal = prevClosing;
      currentItem.closingPrincipal = round(prevClosing - currentItem.principal);
    }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      oldEmisSnapshot.forEach(d => batch.delete(d.ref));

      scheduleToSave.forEach(emi => {
        const { id: _emiId, loanId: _loanId, ...emiData } = emi as any;
  ...
  const openCreateExpenseDialog = async (emi: EMI) => {
    if (!loan) return;
    const emiMonth =
      emi.dueDate && (emi.dueDate as any).toDate ? format(emi.dueDate.toDate(), 'MMMM yyyy') : 'EMI';
    ...
    const expensePayload = {
      ...
      projectId: 'zSOFw2y3jwYStbA3EaL1', // HEAD OFFICE project (hardcoded)
      amount: emi.paidAmount || emi.emiAmount,
    ...
    <>
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Input
                  ...
                  onChange={e => setEditedLoan({ ...editedLoan, loanAmount: Number(e.target.value) })}
                />
              ) : (
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Input
                  ...
                  onChange={e =>
                    setEditedLoan({ ...editedLoan, interestRate: Number(e.target.value) })
                  }
                />
              ) : (
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Input
                  ...
                  onChange={e => setEditedLoan({ ...editedLoan, tenure: Number(e.target.value) })}
                />
              ) : (
            ...
            <div className="space-y-1">
              ...
              {isEditing ? (
                <Input
                  type="date"
                  value={editedLoan.startDate}
                  onChange={e => setEditedLoan({ ...editedLoan, startDate: e.target.value })}
                />
              ) : (
            ...
            <div className="space-y-1">
              ...
              <p className="font-semibold">{loan.linkedBank || 'N/A'}</p>
            </div>
            <div className="space-y-1">
              <Label>EMI</Label>
            ...
            <div className="space-y-1">
              ...
              <p className="font-semibold">{formatCurrency(loan.totalPaid || 0)}</p>
            </div>
            <div className="space-y-1">
              <Label>Outstanding</Label>
              <p className="font-semibold">
                {formatCurrency((loan.loanAmount || 0) - (loan.totalPaid || 0))}
              </p>
            </div>
          </CardContent>
          {isEditing && (
                  ...
                  <TableRow key={emi.id}>
                    ...
                    <TableCell>
                      {isEditing ? (
                        <Input
                          type="number"
                          value={Math.round(emi.principal)}
                          onChange={e => handleEmiScheduleChange(index, 'principal', Number(e.target.value))}
                          className="w-32"
                        />
                      ) : (
                    ...
                    <TableCell>
                      {isEditing ? (
                        <Input
                          type="number"
                          value={Math.round(emi.interest)}
                          onChange={e => handleEmiScheduleChange(index, 'interest', Number(e.target.value))}
                          className="w-28"
                        />
                      ) : (
                    ...
                    <TableCell>
                      {!isEditing &&
                        (emi.status === 'Pending' ? (
                          <Button size="sm" onClick={() => handleMarkAsPaidClick(emi)}>
                            Mark as Paid
                          </Button>
                        ) : (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onSelect={() => handleViewDetailsClick(emi)}>
                                  ...
                                  <TooltipTrigger asChild>
                                    <div className="w-full">
                                      <DropdownMenuItem
                                        onSelect={e => {
                                          e.stopPropagation();
                                          handleEditEmiClick(emi);
                                        }}
                                        disabled={!!emi.expenseRequestNo}
                                      >
                              ...
                              <DropdownMenuItem
                                onSelect={() => openCreateExpenseDialog(emi)}
                                disabled={!!emi.expenseRequestNo}
                              >
                                <FilePlus className="mr-2 h-4 w-4" /> Create Expense
                              </DropdownMenuItem>
                              <DropdownMenuItem
                                onSelect={() => handleMarkAsUnpaid(emi)}
                                disabled={isUpdatingEmi === emi.id || !!emi.expenseRequestNo}
      ...
      <Dialog open={isPayDialogOpen} onOpenChange={setIsPayDialogOpen}>
        <DialogContent className="sm:max-w-md text-left">
          <DialogHeader>
            <DialogTitle>
              {selectedEmi?.status === 'Paid' ? 'Edit' : 'Confirm'} Payment for EMI #{selectedEmi?.emiNo}
            </DialogTitle>
            <DialogDescription>Review the details and confirm the amount paid.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                id="dialog-emi"
                type="text"
                value={formatCurrency((dialogPrincipal || 0) + (dialogInterest || 0))}
                readOnly
            ...
            <div className="space-y-2">
              ...
              <Input
                id="dialog-principal"
                type="text"
                value={formatAsCurrency(dialogPrincipal || 0)}
                onChange={e => setDialogPrincipal(parseCurrency(e.target.value))}
                className="text-left"
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                id="dialog-interest"
                type="text"
                value={formatAsCurrency(dialogInterest || 0)}
                onChange={e => setDialogInterest(parseCurrency(e.target.value))}
                className="text-left"
              />
            </div>
            <div className="space-y-2 pt-4">
              <Label htmlFor="paidAmount" className="text-lg">
                Paid Amount
              </Label>
              <Input
                id="paidAmount"
                type="text"
                value={formatAsCurrency(dialogPaidAmount || 0)}
        ...
        <Dialog open={isViewDetailsOpen} onOpenChange={setIsViewDetailsOpen}>
          <DialogContent>
            ...
            <Table>
              <TableBody>
                <TableRow>
                  ...
                  <TableCell>
                    {users?.find(u => u.id === (selectedEmi as any).paidById)?.name || 'N/A'}
                  </TableCell>
                </TableRow>
                <TableRow>
                ...
                <TableRow>
                  ...
                  <TableCell>{formatCurrency(selectedEmi.paidAmount || 0)}</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell className="font-medium">Principal</TableCell>
                  <TableCell>{formatCurrency(selectedEmi.principal || 0)}</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell className="font-medium">Interest</TableCell>
                  <TableCell>{formatCurrency(selectedEmi.interest || 0)}</TableCell>
                </TableRow>
            ...
            <DialogFooter>
              <Button
                variant="secondary"
                onClick={() => openCreateExpenseDialog(selectedEmi)}
                disabled={isCreatingExpense || !!selectedEmi.expenseRequestNo}
              >
