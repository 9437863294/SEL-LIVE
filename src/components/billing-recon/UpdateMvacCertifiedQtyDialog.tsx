...
interface UpdateMvacCertifiedQtyDialogProps {
  ...
  projectSlug: string;
  onSaveSuccess: () => void;
  onAction?: (taskId: string, action: string | ActionConfig, comment: string, updatedItems: MvacItem[]) => Promise<void>;
}

type EditableItem = MvacItem & { __certStr?: string; __error?: string | null };
...
export function UpdateMvacCertifiedQtyDialog({
  isOpen,
  onOpenChange,
  mvacEntry,
  projectSlug,
  onSaveSuccess,
  onAction,
}: UpdateMvacCertifiedQtyDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && mvacEntry) {
      const cloned: EditableItem[] = JSON.parse(JSON.stringify(mvacEntry.items || []));
      cloned.forEach((it) => {
        it.__certStr = it.certifiedQty ?? it.certifiedQty === 0 ? String(it.certifiedQty) : '';
        it.__error = null;
      });
      setItems(cloned);
      setUploaded((mvacEntry as any).certifiedAttachments || []);
      setSelectedFiles([]);
      setUploadProgress({});
      setIsDragging(false);
    }
  ...
  async function uploadAllSelectedConcurrent(): Promise<CertifiedAttachment[]> {
    if (!mvacEntry || !currentProject || selectedFiles.length === 0) return [];
    setIsUploading(true);
    ...
    const runOne = () =>
      new Promise<void>((resolve, reject) => {
        ...
        const path = `projects/${currentProject.id}/mvacEntries/${mvacEntry.id}/attachments/${Date.now()}-${file.name}`;
  ...
  const handleSave = async () => {
    if (!mvacEntry || !currentProject) return;
    ...
    const payloadItems: MvacItem[] = items.map(({ __certStr, __error, ...rest }) => ({
      ...rest,
      certifiedQty:
    ...
    try {
      const mvacRef = doc(db, 'projects', currentProject.id, 'mvacEntries', mvacEntry.id);
      const mergedAttachments = [...uploaded, ...newAttachments];

      if (onAction) {
        await onAction(mvacEntry.id, 'Verified', 'Verified with edits', payloadItems);
        await updateDoc(mvacRef, { certifiedAttachments: mergedAttachments });
      } else {
        await updateDoc(mvacRef, { items: payloadItems, certifiedAttachments: mergedAttachments });
      }
      ...
      toast({ title: 'Error', description: 'Failed to update the MVAC entry.', variant: 'destructive' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(o) => (canClose() ? onOpenChange(o) : undefined)}>
      <DialogContent className={cn("max-h-[90vh] flex flex-col min-h-0", dialogSizeClass)}>
        <DialogHeader>
          ...
          <DialogDescription>MVAC No: {mvacEntry?.mvacNo}</DialogDescription>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Upload multiple supporting documents (PDFs, images, spreadsheets). They’ll be saved to this MVAC.
          </p>
