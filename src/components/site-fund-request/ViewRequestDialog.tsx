...
export default function ViewRequestDialog({
  ...
  const handleAction = async (action: string | ActionConfig) => {
    ...
    if (actionName === 'Create Expense Request') {
      ...
      setExpenseToCreate({
        ...
        remarks: `Generated from Site Fund Request ${requisition.requisitionId}` || '',
    ...
    setIsLoading(true);
    try {
      const requisitionRef = doc(db, 'siteFundRequests', requisition.id);

      let attachmentData: Attachment | undefined;
      if (file) {
        const currStepName = stepDisplay(currentStep as any);
        const storagePath = `siteFundRequest-actions/${requisition.id}/${currStepName}/${file.name}`;
  ...
  const handlePrint = () => {
    ...
    window.open(
      `/site-fund-request/print/${projectSlug}/${requisition.id}`,
      '_blank'
    );
  };
