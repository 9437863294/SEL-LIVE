...
import ViewRequisitionDialog from './ViewRequestDialog';
...
export default function AllRequestsTab() {
  ...
  const settingsKey = 'siteFundRequests_all';
  ...
  const canCreate = can('Create Requisition', 'Site Fund Request');
  const permCanViewAll = can('View All', 'Site Fund Request');
  const { canViewAll, canWrite, accessibleProjectIds, writableProjectIds, isLoading: accessLoading } = useSFRProjectAccess();
  const effectiveCanAdd = can('Add', 'Site Fund Request.Requests') || can('Create Requisition', 'Site Fund Request') || canWrite;
  ...
  const fetchRequisitions = async () => {
    setIsLoading(true);
    try {
      const reqQuery = query(collection(db, 'siteFundRequests'), orderBy('createdAt', 'desc'));
      ...
      let visible = requisitionsData;
      if (!canViewAll && accessibleProjectIds !== null) {
        visible = requisitionsData.filter(r => accessibleProjectIds.has(r.projectId));
      }
      setRequisitions(visible);
  ...
  const generatePreviewId = async () => {
    try {
      const configRef = doc(db, 'serialNumberConfigs', 'site-fund-request');
  ...
  useEffect(() => {
    ...
    fetchRequisitions();
  }, [toast, accessLoading, canViewAll]);
  ...
  const handleCreateRequest = async (values: FormValues) => {
    ...
    try {
      const workflowRef = doc(db, 'workflows', 'site-fund-request');
      const workflowSnap = await getDoc(workflowRef);
      if (!workflowSnap.exists()) throw new Error("Workflow not configured for Site Fund Request.");
      ...
      if (selectedFiles.length > 0) {
        for (const file of selectedFiles) {
          const storageRef = ref(storage, `siteFundRequests/${newRequisitionId}/${file.name}`);
      ...
      await addDoc(collection(db, 'siteFundRequests'), finalRequisitionData);
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      for (const row of validRows) {
        ...
        await addDoc(collection(db, 'siteFundRequests'), {
          ...tempRequisition,
          requisitionId: newRequisitionId,
          createdAt: Timestamp.now(),
          currentStepId: firstStep.id,
  ...
  const handleEditRequest = async (values: FormValues) => {
    ...
    try {
      const requisitionRef = doc(db, 'siteFundRequests', editingRequisition.id);
  ...
  // Projects visible in the add form — limited to writable projects for restricted users
  const projectsForForm = useMemo(
    () => (canViewAll || accessibleProjectIds === null)
      ? projects
      : projects.filter(p => writableProjectIds.has(p.id)),
    [projects, canViewAll, accessibleProjectIds, writableProjectIds]
  );
