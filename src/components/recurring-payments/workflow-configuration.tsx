...
const renumber = (steps: RecurringWorkflowStep[]) => steps.map((step,index)=>({...step,id:String(index+1)}));
...
export default function RecurringWorkflowConfiguration() {
  ...
  const addStep=()=>setSteps(current=>[...current,{id:String(current.length+1),name:`New Step ${current.length+1}`,description:'',tat:8,assignmentType:'User-based',assignedTo:[],actions:['Approve'],uploadRequired:false}]);
  const removeStep=(id:string)=>setSteps(current=>renumber(current.filter(step=>step.id!==id)));
  ...
  async function save(){for(const step of steps){if(!step.name.trim())return toast({title:'Every step needs a name',variant:'destructive'});if(step.assignmentType==='User-based'&&!(step.assignedTo as string[])[0])return toast({title:`Assign a user to ${step.name}`,variant:'destructive'});if(step.assignmentType==='Amount-based'&&!(step.assignedTo as RecurringAmountAssignee[]).some(x=>x.userId))return toast({title:`Add an amount assignee to ${step.name}`,variant:'destructive'});if(!step.actions.length)return toast({title:`Select at least one action for ${step.name}`,variant:'destructive'});}setSaving(true);try{await setDoc(doc(db,'workflows','recurring-payments-workflow'),{module:'Recurring Payments',steps:renumber(steps),updatedAt:serverTimestamp()});toast({title:'Recurring payment workflow saved'});}catch{toast({title:'Workflow could not be saved',variant:'destructive'});}finally{setSaving(false)}}
