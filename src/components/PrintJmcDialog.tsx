...
const PrintableJmcStyles = ({ orientation }: { orientation: 'portrait' | 'landscape' }) => (
  <style>{`
    @media print {
      ...
      #printable-jmc-sheet.print-landscape {
        width: 297mm;
        height: 210mm;
      }
      ...
      #printable-jmc-sheet, #printable-jmc-sheet * { visibility: visible; }
    }
  `}</style>
);
...
export default function PrintJmcDialog({
  ...
  const handleExport = () => {
    ...
    const detailsRows = [
        [`JMC No.: ${workDetails.jmcNo}`, `DATE: ${workDetails.jmcDate}`],
    ...
    ws['!merges'] = [
      ...
      { s: { r: 8, c: 0 }, e: { r: 8, c: 3 } }, // JMC No & Date
