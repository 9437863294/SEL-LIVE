...
import { ArrowDownAZ, ArrowLeft, ArrowUpAZ, Loader2, Lock, RotateCcw, Save, ShieldAlert, SlidersHorizontal } from 'lucide-react';
import { db } from '@/lib/firebase';
import { useAuth } from '@/components/auth/AuthProvider';
import { useAuthorization } from '@/hooks/useAuthorization';
import { useActivityLogger } from '@/hooks/useActivityLogger';
import { useToast } from '@/hooks/use-toast';
import { SAS_COLLECTIONS, SAS_FIELD_CONTROL_DOC_ID, SAS_SORT_CONTROL_DOC_ID } from '@/lib/site-account-statement';
...
/**
 * Wording for a sort direction, chosen to suit the column.
 *
 * "A to Z" on an Amount column, or "Ascending" on a date, both make a reader stop and translate.
 * Naming what actually happens costs nothing and removes the pause.
 */
function sortDirectionLabel(listKey: SASListKey, fieldKey: string, direction: SASSortDirection): string {
  const type = SAS_LIST_REGISTRY[listKey].fields.find((f) => f.key === fieldKey)?.type ?? 'text';
  if (type === 'number') return direction === 'asc' ? 'Smallest first' : 'Largest first';
  if (type === 'date') return direction === 'asc' ? 'Oldest first' : 'Newest first';
...
export default function SiteAccountFieldControlSettings() {
  ...
  /**
   * Opening sort order per list, held in its own document.
   *
   * Edited here because this is where an administrator already comes to decide how a form
   * behaves, but stored separately from the field-control map so the two saves cannot overwrite
   * each other's shape.
   */
  const [sortDraft, setSortDraft] = useState<SASSortControlDoc>({});
  ...
  useEffect(
    () =>
      onSnapshot(
        ...
        (snapshot) => setSortDraft((snapshot.data() as SASSortControlDoc | undefined) ?? {}),
        // A read failure leaves the registry defaults in force, which is the safe fallback.
        () => undefined,
      ),
    [],
  );

  function setSort(listKey: SASListKey, patch: { field?: string; direction?: SASSortDirection }) {
    setSortDraft((current) => {
      const resolved = resolveSort(listKey, current);
      return { ...current, [listKey]: { ...resolved, ...patch } };
    });
  }

  /** Clearing the entry, rather than writing the default, keeps the document free of noise. */
  function resetSort(listKey: SASListKey) {
    setSortDraft((current) => {
      const next = { ...current };
      delete next[listKey];
      return next;
  ...
  async function save() {
    ...
    try {
      ...
      // Both documents go together, so one Save button means what it says.
      await Promise.all([
        setDoc(doc(db, SAS_COLLECTIONS.settings, SAS_FIELD_CONTROL_DOC_ID), { ...draft, ...stamp }),
        setDoc(doc(db, SAS_COLLECTIONS.settings, SAS_SORT_CONTROL_DOC_ID), { ...sortDraft, ...stamp }),
      ]);
      void log('Update SAS Field Control', { lists: Object.keys(sortDraft).length });
      toast({ title: 'Saved', description: 'Field control and sort order updated.' });
    } catch (e: any) {
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-64">List</TableHead>
                  <TableHead className="w-64">Sort by</TableHead>
                  <TableHead className="w-48">Direction</TableHead>
                  <TableHead className="w-24" />
                </TableRow>
              </TableHeader>
              <TableBody>
                {SAS_LIST_KEYS.map((listKey) => {
                  const list = SAS_LIST_REGISTRY[listKey];
                  const current = resolveSort(listKey, sortDraft);
                  const customised = sortDraft[listKey] !== undefined;
                    ...
                    <TableRow key={listKey}>
                      ...
                      <TableCell>
                        <Select
                          value={current.field}
                          disabled={!canEdit}
                          onValueChange={(value) => setSort(listKey, { field: value })}
                        >
                      ...
                      <TableCell>
                        <Select
                          ...
                          disabled={!canEdit}
                          onValueChange={(value) => setSort(listKey, { direction: value as SASSortDirection })}
                        >
                          <SelectTrigger>
                            <div className="flex items-center gap-2">
                              {ascending ? <ArrowUpAZ className="h-3.5 w-3.5" /> : <ArrowDownAZ className="h-3.5 w-3.5" />}
                              <SelectValue />
                            </div>
                          </SelectTrigger>
                          <SelectContent>
                            {/* Worded per column type — "A to Z" on an amount column reads as nonsense. */}
                            <SelectItem value="asc">
                              {sortDirectionLabel(listKey, current.field, 'asc')}
                            </SelectItem>
                            <SelectItem value="desc">
                              {sortDirectionLabel(listKey, current.field, 'desc')}
                            </SelectItem>
                          </SelectContent>
                        </Select>
                      </TableCell>
                      <TableCell>
                        {canEdit && customised && (
                          <Button variant="ghost" size="sm" onClick={() => resetSort(listKey)}>
